...
# Perform QC!
print("Peforming QC")
print("bash " + str(CodeDir) + "/1_QC_FileArgs.sh " + str(pedfile.name) + " " + str(pedfile.chip))
...
if Mesne:
    ...
    #extract
    os.system("~/bin/plink_linux_1.9_x86_64/plink --file " + pedfile.name + " --cow --keep MesneIndiv.txt --recode --out MesnePasme_" + str(date))
    os.system("sed -i 's/" + pasma + "/Mesne/g' MesnePasme_" + str(date) + ".ped")
    #remove
    os.system("~/bin/plink_linux_1.9_x86_64/plink --file " + pedfile.name + " --cow --remove MesneIndiv.txt --recode --out " + pedfile.name)
...
if merge_ask == "Y":
    ...
    for i in PedFiles:
        ...
        if not os.path.isfile(PLINKDIR + i + '/PLINK_MERGED.ped'):
            mergeChipCommand = "~/bin/plink_linux_1.9_x86_64/plink --file {0} --cow --merge-list {1} --recode --out PLINK_MERGED".format(
        ...
        if os.path.isfile(PLINKDIR + i + '/PLINK_MERGED.ped'):
            mergeChipCommand = "~/bin/plink_linux_1.9_x86_64/plink --file PLINK_MERGED --cow --merge-list {0} --recode --out PLINK_MERGED".format(
                'MergeChip.txt')
            with open('MergeChip.txt', 'w') as csvfile:
                writer = csv.writer(csvfile, delimiter=" ")
