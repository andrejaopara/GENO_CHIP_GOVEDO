...
Parental800=open("/home/andreja/OBDELAVA_GENOTIPOV/GENO_CHIP_GOVEDO/Names_800SNPs.txt").read().strip("\n").split("\n")
...
class pedFile:
    ...
    def extractSNP(self, SNP):
        os.system("~/bin/plink_linux_1.9_x86_64/plink --file " + self.name + " --cow --extract-snp "+ SNP + " --recode --out " + SNP)
 ...
 Do you want to overwrite and proceed? \n Ped and Map files will also be overwritten. [Y/N] ")
        ...
        with open("SNPList.txt", "w") as f:
            for snp in SNPList:
                f.write(snp + "\n")
        os.system("~/bin/plink_linux_1.9_x86_64/plink --file " + self.name + " --cow --extract SNPList.txt --recode --out SNPList")
        ...
        if SNPonChip:
            ...
            with open(Trait + ".txt", "w") as f:
                for snp in TraitSNPs[Trait]:
                    f.write(snp + "\n")
            os.system("~/bin/plink_linux_1.9_x86_64/plink --file " + self.name + " --cow --extract "+Trait+".txt --recode --out " + Trait)
        if not SNPonChip:
        ...
        if number in (100,200,800):
            with open("ParentalSNP_" + str(number) + ".txt", "w") as f:
                for i in eval("SNP"+str(number)+"Sifrant_Dict"):
                    f.write(i + "\n")
            os.system("~/bin/plink_linux_1.9_x86_64/plink --file " + self.name + " --cow --extract ParentalSNP_800.txt --recode --out ParentalSNP800_"+self.sernum)
        else:
